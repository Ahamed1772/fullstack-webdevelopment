...
app.get("/jokes/:id",(req,res) => {
    ...
    let answer = jokes.find((joke) => joke.id === id);
    res.json(answer);
});
...
app.put("jokes/:id",(req,res) => {
    ...
    const finder = jokes.findIndex( (item) = item.id === id);
    jokes[finder] = replacement;
...
const masterKey = "ahamed";
app.delete("/all",(req,res) => {
    const key = req.query.key;
    if (key === masterKey){
        jokes =  [];
        res.sendStatus(200).json({message: "successfully deleted"});
    }else{
        res.sendStatus(404).json({error: "you are unauthorized to delete all the data"});
    }
});
